...
impl Collider3D {
    ...
    pub fn would_collide(&self, rhs: &Collider3D) -> bool {
        if (self.center.x - rhs.center.x).abs() > (self.radius.x + rhs.radius.x) {
            return false;
        }
        if (self.center.y - rhs.center.y).abs() > (self.radius.y + rhs.radius.y) {
            return false;
        }
        if (self.center.z - rhs.center.z).abs() > (self.radius.z + rhs.radius.z) {
            return false;
        }
    ...
    pub fn contains_point(&self, point: &Vec3) -> bool {
        (point.x >= self.center.x - self.radius.x)
            && (point.x <= self.center.x + self.radius.x)
            && (point.y >= self.center.y - self.radius.y)
            && (point.y <= self.center.y + self.radius.y)
            && (point.z >= self.center.z - self.radius.z)
            && (point.z <= self.center.z + self.radius.z)
    }
}
