...
use crate::types::{EcsWorld, Renderable, RigidBody, Transform};
...
pub fn run(world: &EcsWorld) {
    ...
    let (spacetime_renderable, _) = world
        .query::<(&Renderable, &SpacetimeMeshTag)>()
        .next()
        .unwrap();

    let grid_mesh = resource_manager
        .get_any_mesh(spacetime_renderable.mesh)
        .unwrap();
    let shader = resource_manager
        .get_shader(spacetime_renderable.shader)
        .unwrap();

    let influences: Vec<_> = world
        .query::<(&Transform, &RigidBody, &SpacetimeCurvature)>()
